  ...
  <div class="tabcontent">
    <v-row>
      ...
      <v-col cols="6" align="right">
        <v-btn color="primary" @click="onClickObjectAdd">Добавить объект</v-btn>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    onClickObjectAdd(){
      this.$parent.addTab('Добавить объект', 0, 'ObjectAdd', {id: 0});
    },
    onClickObjectUpdate(item){
      this.$parent.addTab('Изменить объект '+ item.adress, item.IdHouse, 'ObjectAdd', {id: item.IdHouse});
    },
    changeFilter(){
      /*this.$store.dispatch('GET_TSG_ALL_LIST', {IsDeleted: this.isDeleted==true?1:0}).then(response => {
        this.tsg = response.data;
        this.tsgItem = [];
        this.tsg.forEach(item => {
          if(item.IsDeleted==0)
